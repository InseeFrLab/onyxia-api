...
public class XGeneratedProcessor {
    ...
    @Autowired
    private XGeneratedReader xGeneratedReader;

    @Autowired
    private XGeneratedIterator xGeneratedIterator;

    @Autowired
    private XGeneratedInjector xGeneratedInjector;

    public XGeneratedContext readContext(Pkg pkg) {
        XGeneratedContext xGeneratedContext = new XGeneratedContext();
        pkg.getConfig().getProperties().getProperties().entrySet().stream().forEach((entry) -> {
            xGeneratedReader.readXGenerated(Arrays.asList(entry.getKey()),entry.getValue(),xGeneratedContext);
        });
        return xGeneratedContext;
    }

    public Map<String,String> process(XGeneratedContext xGeneratedContext, XGeneratedProvider provider) {
    ...
    public void injectIntoContext(Map<String,Object> target, Map<String,String> xGeneratedValues) {
