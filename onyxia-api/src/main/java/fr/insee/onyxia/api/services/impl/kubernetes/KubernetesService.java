...
public class KubernetesService {
    ...
    public String createNamespace(Region region, String namespaceId, Owner owner) {
        ...
        // Label onyxia_owner is not resilient if the user has "namespace admin" role
        // scoped to his namespace
        ...
        if (kubClient.namespaces().withName(namespaceId).get() != null) {
            return namespaceId;
        }

        Namespace namespaceToCreate = kubClient.namespaces().create(new NamespaceBuilder().withNewMetadata()
                .withName(namespaceId).addToLabels("onyxia_owner", owner.getId()).endMetadata().build());

        RoleBinding bindingToCreate = kubClient.rbac().roleBindings().inNamespace(namespaceId)
                .create(new RoleBindingBuilder().withNewMetadata().withLabels(Map.of("createdby", "onyxia"))
                        ...
                        .withName("admin").endRoleRef().build());
