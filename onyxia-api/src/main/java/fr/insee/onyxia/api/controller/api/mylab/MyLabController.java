...
public class MyLabController {
   ...
   private CustomMetrics metrics;
    ...
    public Group getGroup(
        @RequestParam(value="groupId",required = false) String id) throws JsonParseException, JsonMappingException, IOException {
        String uri = "";
        ...
        Request requete =
            new Request.Builder()
                .url(
                   MARATHON_URL
                        + "/v2/groups/users/"
                        + userProvider.getUser().getIdep() + uri + "?"
                        + "embed=group.groups" + "&" + "embed=group.apps" + "&" + "embed=group.apps.tasks" + "&"
                        + "embed=group.apps.counts" + "&" + "embed=group.apps.deployments" + "&"
                        + "embed=group.apps.readiness" + "&" + "embed=group.apps.lastTaskFailure" + "&"
                        + "embed=group.pods" + "&" + "embed=group.apps.taskStats")
                .build();
        Response response = HttpUtils.CLIENT.newCall(requete).execute();
    ...
    public UniversePackage getPackage(
    ...
    @GetMapping("/app")
    public @ResponseBody String getApp(
        @RequestParam("serviceId") String id) throws JsonParseException, JsonMappingException, IOException {

       String url =  MARATHON_URL
          + "/v2/apps/users/"
          + userProvider.getUser().getIdep() + "/" + id
    ...
    @DeleteMapping("/app")
    public Result destroyApp(
        @RequestParam("serviceId") String id) throws MarathonException {
    ...
    @PutMapping("/app")
    public App publishService(
       @RequestBody CreateServiceDTO requestDTO) throws JsonProcessingException, IOException, MarathonException, Exception {
        ...
        long nbInvalidations = admissionControllers.stream().map(admissionController -> admissionController.validateContract(user, pkg, requestDTO.getOptions())).filter(b -> !b).count();
            ...
            if (app.getLabels().containsKey("HAPROXY_1_VHOST")) {
                app
                    .addLabel(
                        "ONYXIA_URL",
                        "https://"
                            + app.getLabels().get("HAPROXY_0_VHOST") + ",https://"
    ...
    @PutMapping("/group")
    public Collection<App> publishGroup(
       @RequestBody CreateServiceDTO requestDTO) throws JsonProcessingException, IOException, MarathonException, Exception {
